...
class Datasheet(WebsiteGenerator):
	def get_context(self, context):
		...
		context["comments"]=frappe.db.sql(""" select C.content,C.reference_name,C.reference_doctype,C.comment_by,C.creation from `tabComment` as C left join `tabDatasheet`  as D on reference_name = D.name where C.reference_name = %s and C.content is NOT NULL order by C.creation desc limit 10""",(context.doc.name),as_dict=True)
