...
class Project(WebsiteGenerator):
	def get_context(self, context):
		...
		if context['access_allowed']:
			...
			context['userinfo']=frappe.db.sql(""" select P.name,U.user_image,U.full_name from `tabUser` as U  left join `tabProject` as P on  U.name = P.owner where P.name = %s """,(self.name),debug=1)
