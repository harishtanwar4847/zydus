...
def get_context(context):
    ...
    if context['access_allowed']:
        ...
        (select docstatus, "Project" as doctype,P.description, P.route,P._liked_by as liked_by,P.brand,P.name,P.title,concat(P.month," ",P.year) as month_year,P.agency,P._user_tags,P.project_type as type from `tabProject` as P   where {P_and_where_clauses} and ({P_or_where_clauses})
        union
        select docstatus, "Datasheet" as doctype,"" as description, D.route, D._liked_by as liked_by,D.brand,D.name,D.title,concat(D.month," ",D.year) as month_year,D.agency,D._user_tags,D.data_type as type from `tabDatasheet`as D   where {D_and_where_clauses} and ({D_or_where_clauses})) as T left join `tabBrand` as B on T.brand = B.name left join `tabView Log` as V on V.reference_name= T.name and V.reference_doctype= T.doctype and T.docstatus = 1 group by T.name """.format(**clauses), as_dict=True, debug=1)
