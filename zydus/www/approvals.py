...
def get_context(context):
    ...
    if context['access_allowed']:
        ...
        context['approval_approved'] = frappe.db.sql("""select T.modified,T.doctype,T.owner,T.route,T.name,T.title,T.type,T.agency,T.month_year,B.color,T.brand,T.workflow_state,U.full_name from (select modified,docstatus, owner, "Project" as doctype, P.route,P.brand, P.project_type as type,P.agency,P.workflow_state ,P.name,P.p_title as title,concat(P.month," ",P.year) as month_year from `tabProject` as P    union         select modified,docstatus, owner, "Datasheet" as doctype, D.route,D.brand,D.data_type as type,D.agency,D.workflow_state ,D.name,D.d_title as title,concat(D.month," ",D.year) as month_year from `tabDatasheet`as D) as T left join `tabBrand` as B on T.brand = B.name left join `tabUser`as U on U.name = T.owner where T.workflow_state = "approved"  order by modified desc limit 10""",as_dict=True,debug=1)
