...
def get_context(context):
    ...
    if context['admin_access_allowed']:
        ...
        context['approved'] = frappe.db.sql("""select T.modified,T.doctype,T.owner,T.route,T.name,T.title,T.liked_by,T.type,T.agency,T.month_year,B.color,T.brand,T.workflow_state,U.full_name from (select modified,docstatus, owner, "Project" as doctype, P.route,P.brand, P.project_type as type,P.agency,P.workflow_state,P._liked_by as liked_by ,P.name,P.p_title as title,concat(P.month," ",P.year) as month_year from `tabProject` as P    union         select modified,docstatus, owner, "Datasheet" as doctype, D.route,D.brand,D.data_type as type,D.agency,D.workflow_state,D._liked_by as liked_by ,D.name,D.d_title as title,concat(D.month," ",D.year) as month_year from `tabDatasheet`as D) as T left join `tabBrand` as B on T.brand = B.name left join `tabUser`as U on U.name = T.owner where T.workflow_state = "approved"  order by modified desc limit %(limit)s offset %(offset)s""",{'limit': context['approved_page_length'], 'offset': context['approved_page_offset']} ,as_dict=True)
        context['approved_count'] = frappe.db.sql("""  select count(T.name) as count from (select P.name from `tabProject` as P where P.workflow_state = "approved"        union         select D.name from `tabDatasheet` as D where D.workflow_state = "approved") as T """,as_dict=1)[0]['count']
        ...
        select modified,docstatus, owner, "Datasheet" as doctype, D.route,D.brand,D.data_type as type,D._liked_by as liked_by,D.agency,D.workflow_state ,D.name,D.d_title as title,concat(D.month," ",D.year) as month_year from `tabDatasheet`as D where D.workflow_state="pending" ) as T left join `tabBrand` as B on T.brand = B.name left join `tabUser`as U on U.name = T.owner  order by modified desc limit %(limit)s offset %(offset)s""",{'limit': context['pending_page_length'], 'offset': context['pending_page_offset']} ,as_dict=True)
        context['pending_count'] = frappe.db.sql("""  select count(T.name) as count from (select P.name from `tabProject` as P where P.workflow_state = "pending"        union         select D.name from `tabDatasheet` as D where D.workflow_state = "pending") as T """,as_dict=1)[0]['count']
