...
def get_context(context):
    ...
    if context['access_allowed']:
        ...
        context['my_uploads'] = frappe.db.sql("""  select T.doctype,T.owner,T.route,T.liked_by,T.name,T.title,T.month_year,B.color,T.brand from          (select docstatus, owner, "Project" as doctype, P.route,P._liked_by as liked_by,P.brand,P.name,P.title,concat(P.month," ",P.year) as month_year from `tabProject` as P where owner= %s          union         select docstatus, owner, "Datasheet" as doctype, D.data_type, D._liked_by as liked_by,D.brand,D.name,D.title,concat(D.month," ",D.year) as month_year from `tabDatasheet`as D where owner = %s) as T left join `tabBrand` as B on T.brand = B.name and T.docstatus = 1   limit 10; """, (frappe.session.user,frappe.session.user),as_dict=1,debug=1)
