...
def get_context(context):
    ...
    #     select modified,docstatus, owner, "Datasheet" as doctype, D.route,D.brand,D.data_type as type,D._liked_by as liked_by,D.agency,D.workflow_state ,D.name,D.d_title as title,concat(D.month," ",D.year) as month_year from `tabDatasheet`as D where D.workflow_state="pending" ) as T left join `tabBrand` as B on T.brand = B.name left join `tabUser`as U on U.name = T.owner  order by modified desc limit 6 """,as_dict=True)   
    # else:
    ...
    context['trending_now_list'] = frappe.db.sql("""
        ...
        select modified,docstatus, owner, "Datasheet" as doctype, D.route,D.brand,D.data_type as type,D._liked_by as liked_by,D.agency,D.workflow_state ,D.name,D.d_title as title,concat(D.month," ",D.year) as month_year from `tabDatasheet`as D where D.workflow_state="{where_clause}" ) as T left join `tabBrand` as B on T.brand = B.name left join `tabUser`as U on U.name = T.owner  {viewlog_left_join} order by {order_by} desc limit 6 """.format(view_count=view_count,where_clause=where_clause,viewlog_left_join=viewlog_left_join,order_by=order_by),as_dict=True)   
    where_clause = ""
    if context['admin_access_allowed']:
