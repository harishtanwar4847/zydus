...
def get_context(context):
    ...
    if context['access_allowed']:
        ...
        union
        select docstatus, "Datasheet" as doctype, D.data_type, D._liked_by as liked_by,D.brand,D.name,D.title,concat(D.month," ",D.year) as month_year from `tabDatasheet`as D) as T left join `tabBrand` as B on T.brand = B.name left join `tabView Log` as V on V.reference_name= T.name and V.reference_doctype= T.doctype and T.docstatus = 1 group by T.name having view_count > 0 order by view_count desc limit 6 """,as_dict=True)
        for trending_now in context['trending_now_list']:
            ...
            trending_now['liked_by'] = ','.join(json.loads(trending_now['liked_by'] or '[]'))

        context['my_uploads'] = frappe.db.sql(""" select T.doctype,T.route,T.liked_by,T.name,T.title,T.month_year,B.color,T.brand from 
        (select docstatus, owner, "Project" as doctype, P.route,P._liked_by as liked_by,P.brand,P.name,P.title,concat(P.month," ",P.year) as month_year from `tabProject` as P 
        union
        select docstatus, owner, "Datasheet" as doctype, D.data_type, D._liked_by as liked_by,D.brand,D.name,D.title,concat(D.month," ",D.year) as month_year from `tabDatasheet`as D) as T left join `tabBrand` as B on T.brand = B.name and T.docstatus = 1 and T.owner = %s group by T.name  limit 10 """, (frappe.session.user),as_dict=1)
        for my_upload in context['my_uploads']:
            my_upload['attachments'] = get_attachments(my_upload.doctype,my_upload.name)
            my_upload['liked_by'] = ','.join(json.loads(my_upload['liked_by'] or '[]'))
