...
def get_context(context):
    ...
    if context['access_allowed']:
        ...
        context['my_uploads'] = frappe.db.sql(""" select P.name,route,title,color,brand,concat(month," ",year)as month_year from `tabProject` P left join `tabBrand` on P.brand = tabBrand.name group by P.name  limit 10 """,as_dict=1)
