...
def get_context(context):
    ...
    if context['access_allowed']:
        ...
        context["reminders"] = frappe.db.sql(""" select U.user_image,T.name,T.title,T.description,T.owner,T.modified_by,T.date from `tabToDo` as T left join `tabUser` as U on T.owner = U.owner where status = "open" order by date asc limit 5 """,as_dict=1,debug=1)
